    ...
    public RobotContainer() {
     ...
     private void configureBindings() {
        ...
        .whileTrue(new IntakeCoralToCoralHand(m_inlet, m_coralHand, m_coralSensor));
         ...
         // If y is pressed, run CoralShoulder 
         ...
         .toggleOnTrue(new SetCoralShoulder(m_coralShoulder,-60.0).alongWith(new SetElevator(m_elevator,-1.0)));
        
         // If POVUP is pressed, run CoralShoulderand the elevator in parallel
         m_operatorController.povRight()
        .toggleOnTrue(new SetElevator(m_elevator,-1.0));

         // If povDown is pressed, run L1 Score: Set coralshoulder, wait then run coralhand.
         m_operatorController.povDown()
         .toggleOnTrue(new SetCoralShoulder(m_coralShoulder,-15.0).alongWith(new WaitCommand(0.3).andThen(new RunCoralHand(m_coralHand, 100.0))));
         
         
         // If povright is pressed, run alternate  L1 Score: Set coralshoulder, wait then run coralhand.
         // m_operatorController.povRight()
         // .toggleOnTrue(new L1Score(m_coralShoulder, m_coralHand, m_coralSensor));
